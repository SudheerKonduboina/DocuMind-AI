...
def fix_imports_in_file(filepath):
    ...
    content = re.sub(
        r"from \.base import", "from backend.database.base import", content
    )
    content = re.sub(
        r"from \.session import", "from backend.database.session import", content
    )
    content = re.sub(
        r"from \.init_db import", "from backend.database.init_db import", content
    )
...
def fix_all_imports(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        dirs[:] = [
            d for d in dirs if d not in ["__pycache__", ".pytest_cache", "venv", "env"]
        ]
