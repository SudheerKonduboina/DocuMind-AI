...
@pytest.mark.asyncio
async def test_rate_limit_user_id_from_client(
    mock_request, mock_call_next, monkeypatch
):
    ...
    mock_rate_limiter.is_allowed.assert_called_with(
        "127.0.0.1", settings.RATE_LIMIT_REQUESTS, settings.RATE_LIMIT_PERIOD
    )


@pytest.mark.asyncio
async def test_rate_limit_user_id_from_header(
    mock_request, mock_call_next, monkeypatch
):
    monkeypatch.setattr("backend.core.rate_limiting.settings.RATE_LIMIT_ENABLED", True)
    ...
    assert response.body == b"ok"
    mock_rate_limiter.is_allowed.assert_called_with(
        "test_user", settings.RATE_LIMIT_REQUESTS, settings.RATE_LIMIT_PERIOD
    )
...
async def test_rate_limit_user_id_unknown(mock_request, mock_call_next, monkeypatch):
    ...
    mock_rate_limiter.is_allowed.assert_called_with(
        "unknown", settings.RATE_LIMIT_REQUESTS, settings.RATE_LIMIT_PERIOD
    )
