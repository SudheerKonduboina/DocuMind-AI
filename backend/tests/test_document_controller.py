...
async def test_upload_document_file_too_large(controller, monkeypatch):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await controller.upload_document(
            file=mock_file, title="title", user_id="user1", db=MagicMock()
        )
...
async def test_upload_document_success_audio(controller, monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.modules.document.document_controller.DocumentService", mock_service_cls
    )

    res = await controller.upload_document(
        file=mock_file, title="title", user_id="user1", db=MagicMock()
    )
    assert res == {"id": "doc1"}
