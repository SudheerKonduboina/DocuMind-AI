...
class MediaService:
    ...
    async def transcribe_media(
        self, file_path: str, document_id: str, language: str = "en"
    ) -> TranscriptWithSegmentsResponse:
        """
        ...
        try:
            ...
            logger.info(
                f"Whisper transcription completed | segments_count: {len(segments_data)}"
            )
        ...
        transcript_data = TranscriptCreate(
            document_id=document_id, full_text=transcript_text, language=language
        )
    ...
    def _create_timestamped_segments(
        self, transcript_id: str, whisper_segments: List[dict]
    ) -> List:
        """
    ...
    def get_transcript_segments(
        ...
        if start_time is not None and end_time is not None:
            segments = self.segment_repo.get_by_time_range(
                transcript.id, start_time, end_time
            )
        else:
            segments = self.segment_repo.get_by_transcript(transcript.id)
