...
from backend.modules.playback.playback_schemas import (
...
class PlaybackService:
    ...
    async def get_playback_segment(
        self, document_id: str, user_id: str, request: PlaybackSegmentRequest
    ) -> PlaybackSegmentResponse:
        """
        ...
        end_time = min(
            transcript.duration_seconds or request.timestamp + request.duration,
        ...
        s3_url = None
        if document.s3_key:
            s3_url = await s3_service.generate_presigned_url(
                document.s3_key, expiration=3600
            )
