...
class SummarizationService:
    ...
    async def generate_summary(
        self, document_id: str, user_id: str, summary_type: str = "auto"
    ) -> SummaryResponse:
        """
        ...
        if existing_summary:
            summary = self.summary_repo.update(
                existing_summary, SummaryUpdate(content=summary_text)
            )
        else:
            summary_data = SummaryCreate(
                document_id=document_id,
    ...
    async def regenerate_summary(
        ...
        return await self.generate_summary(
            document_id, user_id, summary_type="on_demand"
        )
