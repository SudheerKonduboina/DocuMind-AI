...
class ChatbotService:
    ...
    def create_chat(self, chat_data: ChatCreate, user_id: str) -> ChatResponse:
    ...
    async def send_message(
        self, chat_id: str, content: str, user_id: str
        ...
        if len(content) > self.MAX_MESSAGE_LENGTH * 2:
            content = (
                content[: self.MAX_MESSAGE_LENGTH * 2] + "\n...[message truncated]"
            )
        ...
        history = [
            {
                "role": msg.role,
                "content": (
                    msg.content[: self.MAX_MESSAGE_LENGTH] + "...[truncated]"
        ...
        if chat.document_id:
            context, context_timestamp, source_document = await self._retrieve_context(
                chat.document_id, content, user_id
            )
        ...
        if estimated_tokens > self.MAX_TOTAL_TOKENS_ESTIMATE:
            ...
            api_messages[0]["content"] = (
                system_content[:max_system_chars] + "\n...[truncated]"
            )
            # If still too large, drop history messages from the start
            while (
                len(api_messages) > 2
                and sum(len(m["content"]) for m in api_messages) // 3
        ...
        metadata = (
            ...
            if timestamps or context_timestamp
            else None
        )
        self.message_repo.create_with_metadata(
            chat_id, "assistant", full_response, metadata
        )
    ...
    async def _retrieve_context(
        self, document_id: str, query: str, user_id: str
    ) -> tuple:
        """
        ...
        if document and document.file_type in ["audio", "video"]:
            ...
            if transcript:
                ...
                if segments:
                    ...
                    return (
                        context,
                        segments[0].start_time if segments else None,
                        str(document_id),
                    )
